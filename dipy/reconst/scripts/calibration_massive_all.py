...
nib.save(nib.Nifti1Image(m, img.get_affine()), join(do, ''.join((f, '_single_fiber_mask_t005_nomask.nii'))))
import scipy.io
scipy.io.savemat(join(do, ''.join((f, '_single_fiber_response_t005_nomask.mat'))), mdict={'r_sh': response})
np.save(join(do, ''.join((f, '_single_fiber_response_t005_nomask'))), response)
